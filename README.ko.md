...
**Gemma 4 26B MoE** (Apple Silicon / MLX) 기반, **Claude**를 사실 정확성을 위한 선택적 클라우드 레이어로 사용.
...
26B 모델을 챗봇이 아닌 친구처럼 행동하게 만드는 방법. 4개 버전, 10개 페르소나 테스트. 핵심 발견: 큰 모델일수록 규칙이 아닌 자연어 문단이 필요. 내부 루브릭으로 여러 페르소나에 걸쳐 평가.

### [chat-like-human-memory](https://github.com/nskit-io/chat-like-human-memory)
**9차원 감정 + 성격 진화 + 3단계 기억.**

대화 중 감정이 변동하고 자연스럽게 감쇄. 성격은 주 단위로 **decay/commit** 메커니즘으로 진화. 기억은 원본 → 이벤트 요약 → 흐릿한 키워드로 압축 — 실제 사람의 기억처럼.
...
- **분산 판단 (judge + core)**: 경량 모델이 분류 작업 (수신 판단, 명령 인식) 담당, 26B 모델은 실제 대화 생성에만 사용. 4단 폴백 체인으로 **LLM 환각 폴백을 원천 차단** — 판단 실패 시 룸에 시스템 메시지 표시 (가짜 응답 절대 안 만듦).
- **Source-agnostic 상태 변경**: 마스터 명령, 자율 결정, UI 이벤트 모두 단일 상태 변경 경로로 처리. "왜" 만 trace 에 남고 "무엇" 은 하나의 코드 경로로 통일.
- **Activity Density Tier (T1~T4)**: 리소스 비용이 active user 수에 선형 비례.
  - T1 (최근 터치): 실시간, full LLM
  - T2 (짧은 유휴): 이벤트 기반
  - T3 (중간 유휴): 최소 LLM
  - T4 (장기 유휴): **LLM 콜 0** — 로직 기반 기억만 매일 축적. 휴면 아바타는 비용이 거의 제로
- **Rest → compaction**: 아바타가 쉬는 상태 진입 시 (마스터 허락 or 자동 idle) **스스로** 장기 기억 정리. 상태 필드가 단순 톤 힌트가 아닌 **실제 행동 트리거**.
- **Daily narrative backbone**: 장기 유휴 아바타도 매일 기억 이벤트 1건 축적. 사용자 복귀 시 몰아서 생성하지 않음 — "방학 숙제 몰아 하는" 실수 방지.
- **Trace 기반 관측**: 모든 판단은 전용 trace 테이블에 기록. "왜 비비가 그때 대답 안 했지?" 타임라인 조회 가능.

프로토타입으로 구축해 장시간 내부 스트레스 실행으로 검증 (벤치마크가 아닌 참고용). 룸 브로드캐스트, 자율 친구 방문, 주사위 기반 디스패치는 이 레이어 위에서 탐색해 본 방향.

## 숫자로 보는 NVatar

- **캐릭터 품질** 10인 페르소나에 걸쳐 내부 루브릭으로 평가
- **로컬 컨텍스트 분류** — 핫패스에서 클라우드 왕복 없음
- **9차원** 연속 감정 추적 (호기심 포함)
- **3단계** 기억 시스템 (rest 상태 진입 시 자동 압축)
- **4단계** Activity Density — 휴면 사용자 비용 거의 0
- **장시간 내부 스트레스 실행** — 장기 룸 시뮬레이션 (벤치마크가 아닌 참고용)
...
- **공백**: 로컬 AI 프라이버시 + 깊은 인지 아키텍처 + 3D 아바타를 결합한 제품이 드묾

### 우리가 만든 것
- 10타입 컨텍스트 라우팅 + 로컬/클라우드 분리
- 시간 감쇄 기반 성격 진화 (학술 개념 → 실동작 구현)
- 9차원 연속 감정 추적, 3D 아바타와 통합
- 자율 이동 가능한 3D 룸 환경
